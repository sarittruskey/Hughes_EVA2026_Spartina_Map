...
# The lines below check the expected directory structure required to run this script

required_dirs <- c("data", "map_figs") # this Rmd should be housed within a directory that has the internal directories: data (folder with the input data to produce inset figures) and map_figs (folder where output map plots are written to)
missing_dirs <- required_dirs[!dir.exists(required_dirs)] # if either data or map_figs is missing, it will be saved to missing_dirs vector

if (length(missing_dirs) > 0) { # this if-statement will produce the following error message that includes which of the internal directories you are missing (if applicable) 
  stop(
...
base_map <- ggplot() +
  ...
  coord_sf( # this controls map extent: adjust these numbers as wanted to make the plot more or less zoomed in to the sites
    xlim = c(-78, -69), # x is longitude (you can adjust these as desired)
    ylim = c(37, 43.5), # y is latitude (you can adjust these as desired)
    expand = FALSE) + 
