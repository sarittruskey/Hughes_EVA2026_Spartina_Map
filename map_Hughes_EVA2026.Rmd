...
inset_map_ROW <- inset_map_func( 
  long_range = c(-70.87, -70.78),  
  lat_range = c(42.72, 42.78),
  scale_bar_loc = "tl",
  scale_text_cex = 1.5) + 
...
inset_map_ROW_sitenums_no3sa <- inset_map_ROW + geom_sf_text( # this function will add points to inset_map_ROW plot as text
  data = pts_ll[pts_ll$site_num_1to9 != "3sa",], # will add 3sa separately because label is really close to subsite 2 otherwise
...
inset_map_ROW_sitenums <- inset_map_ROW_sitenums_no3sa + geom_sf_text( # this function will add points to inset_map_ROW plot as text
  data = pts_ll[pts_ll$site_num_1to9 == "3sa",], #adding in 3sa label
  aes(label = site_num_1to9),
  size = 5, #size of points
...
inset_map_RUMBEL <- inset_map_func( 
  ...
  scale_bar_loc = "tr", #bumped scale to top right
  scale_text_cex = 1.5) + 
